...
local WATERMARK_IMAGE_ID = "rbxassetid://0" -- Placeholder for watermark image
...
function gui.dashboard.expand()
	...
	local dashboardHeight = math.min(320 * scale, maxHeight)
	
	local tweenService = game:GetService("TweenService")
	local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	
	if dashboardState.window then
		local goal = {Size = UDim2.new(0, 320 * scale, 0, dashboardHeight)}
...
function gui.dashboard.collapse()
	...
	if dashboardState.window then
		local goal = {Size = UDim2.new(0, 320 * scale, 0, 22 * scale)}
		local tween = tweenService:Create(dashboardState.window, tweenInfo, goal)
		tween:Play()
...
function gui.dashboard.init()
	...
	dashboardFrame.Size = UDim2.new(0, dashboardWidth, 0, titleBarHeight)
	dashboardFrame.Position = UDim2.new(0, 10 * scale, 1, -(titleBarHeight + 10 * scale))
	...
	titleBar.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			startPos = dashboardFrame.Position.Y.Offset
			updateInteraction()
		end
	end)
	...
	game:GetService("UserInputService").InputChanged:Connect(function(input)
		if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			local delta = dragStart - input.Position.Y
			
			if delta > 50 * scale and not dashboardState.isExpanded then
				gui.dashboard.expand()
				dragging = false
			elseif delta < -50 * scale and dashboardState.isExpanded then
				gui.dashboard.collapse()
				dragging = false
			end
		end
	...
	commandScrollFrame.Size = UDim2.new(1, -8 * scale, 1, -(62 * scale))
	...
	local watermarkFrame = Instance.new("ImageLabel")
	watermarkFrame.Name = "Watermark"
	watermarkFrame.Size = UDim2.new(0, dashboardWidth, 0, 320 * scale)
	watermarkFrame.Position = UDim2.new(0, 10 * scale, 1, -(320 * scale + 10 * scale))
	watermarkFrame.BackgroundTransparency = 1
	watermarkFrame.Image = WATERMARK_IMAGE_ID
	watermarkFrame.ImageTransparency = 0
	...
	task.spawn(function()
		...
		local fadeTween = tweenService:Create(watermarkFrame, fadeInfo, {ImageTransparency = 1})
		fadeTween:Play()
		
		fadeTween.Completed:Connect(function()
			watermarkFrame:Destroy()
			uiElements.watermarkFrame = nil
			dashboardState.watermarkShown = true
	...
	task.spawn(function()
		while dashboardState.window do
			...
			if dashboardState.isExpanded and not dashboardState.isTyping then
				local timeSinceInteraction = tick() - dashboardState.lastInteraction
				if timeSinceInteraction >= 5 then
					gui.dashboard.collapse()
				end
			end
		end
