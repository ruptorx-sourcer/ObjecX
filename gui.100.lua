...
local function updateScrollSize()
	...
	-- FIX #2: Add extra padding at bottom so last item is fully visible
	local scale = getScale()
...
function gui.dashboard.expand()
	...
	if dashboardState.window then
		...
		tween.Completed:Connect(function()
			...
			-- FIX #3: Enable timeout only after animation completes
...
function gui.dashboard.init()
	...
	-- FIX #3: Disable timeout during watermark animation
	...
	task.spawn(function()
		...
		local fadeTween = tweenService:Create(watermarkFrame, fadeInfo, {BackgroundTransparency = 1})
	...
	dashboardFrame.Position = UDim2.new(1, -dashboardWidth - 10 * scale, 1, -titleBarHeight)
	...
	-- FIX #3: Track scrolling as activity
	...
	-- NOW fade the watermark after dashboard is ready
	task.spawn(function()
		task.wait(1.25)
		...
		if uiElements.watermarkFrame then
			...
			-- Fade only the content, not the frame
			if imageLoaded then
				tweenService:Create(imageLabel, fadeInfo, {ImageTransparency = 1}):Play()
			else
				tweenService:Create(fallbackLabel, fadeInfo, {TextTransparency = 1}):Play()
			end
			
			-- Fade the background too
			local bgFade = tweenService:Create(watermarkFrame, fadeInfo, {BackgroundTransparency = 1})
			bgFade:Play()
			
			bgFade.Completed:Connect(function()
				watermarkFrame:Destroy()
				uiElements.watermarkFrame = nil
				-- Dashboard is already expanded, just enable timeout
				dashboardState.isExpanded = true
				dashboardState.timeoutActive = true
				updateInteraction() -- Reset the 5s timer
			end)
		end
	end)
