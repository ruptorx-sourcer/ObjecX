...
function Certificate.view(signedCert, publicKey)
    ...
    local certChunks = stringToChunks(certPart, 2)
...
-- GUI Loader
oldprint("[SYSMAINT] GUI loading...")
local guiModule = nil

if CACHE_MISS == 1 then
	oldprint("[SYSMAINT] Cache miss, fetching GUI from network")
	guiModule = fetchAndLoadGui()
	
	if not guiModule then
		warn("[SYSMAINT] Failed to fetch GUI from network")
		traceback("GUILoader", "Failed to fetch GUI from network")
	end
else
	oldprint("[SYSMAINT] Attempting to load GUI from cache")
	guiModule = loadGuiFromCache()
	
	if not guiModule then
		oldprint("[SYSMAINT] Cache load failed, trying network")
		guiModule = fetchAndLoadGui()
	end
end

if not guiModule then
	warn("[SYSMAINT] All GUI loading attempts failed, trying cache as last resort")
...
local success, err = pcall(function()
	if guiModule then
		oldprint("ObjecX Core v" .. oldtostring(CVersion) .. " loaded successfully with 26-bit RSA certificate verification")
	else
...
oldprint("[SYSMAINT] ObjecX startup complete")
