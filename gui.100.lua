...
function gui.dashboard.init()
	...
	-- Create the dashboard frame (visible but empty, watermark overlays it)
	...
	dashboardFrame.Visible = true  -- Visible from start (but empty)
	...
	imageLabel.BackgroundTransparency = 1
	imageLabel.Image = "rbxassetid://82808466179314"
	...
	task.spawn(function()
		...
		if imageLoaded then
			...
			imageFadeTween.Completed:Connect(function()
				...
				-- Populate commands AFTER watermark fades
				gui.dashboard.populateCommands()
				gui.dashboard.search("")
				
				-- Enable timeout now that everything is loaded
				dashboardState.timeoutActive = true
			end)
		else
	...
	searchContainer.Visible = false  -- Hidden until watermark fades
	...
	commandScrollFrame.Visible = false  -- Hidden until watermark fades
	...
	-- DON'T populate commands yet - wait for watermark to fade
