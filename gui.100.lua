...
local function updateInteraction()
	dashboardState.lastInteractionTime = tick()
end
...
function gui.dashboard.expand()
	if dashboardState.isExpanded then return end
	
	dashboardState.isExpanded = true
	updateInteraction()
	
	if dashboardState.containerFrame then
		local scale = getScale()
		local screenSize = workspace.CurrentCamera.ViewportSize
		...
		local tween = tweenService:Create(dashboardState.containerFrame, tweenInfo, {
			Position = UDim2.new(1, -dashboardWidth - (10 * scale), 1, -dashboardHeight - (10 * scale)),
...
function gui.dashboard.collapse()
	if not dashboardState.isExpanded then return end
	...
	if dashboardState.containerFrame then
		local scale = getScale()
		local dashboardWidth = 320 * scale
		local titleBarHeight = 30 * scale
		
		local tweenService = game:GetService("TweenService")
		local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
		
		local tween = tweenService:Create(dashboardState.containerFrame, tweenInfo, {
			Position = UDim2.new(1, -dashboardWidth - (10 * scale), 1, -(titleBarHeight + 10 * scale)),
...
function gui.dashboard.init()
	...
	watermarkFrame.Name = "DashboardWatermark"
	watermarkFrame.Size = UDim2.new(0, dashboardWidth, 0, dashboardHeight)
	watermarkFrame.Position = UDim2.new(1, -dashboardWidth - (10 * scale), 1, -dashboardHeight - (10 * scale))
	watermarkFrame.BackgroundColor3 = palette.windowBg
	watermarkFrame.BackgroundTransparency = 0
	watermarkFrame.BorderSizePixel = 1
	watermarkFrame.BorderColor3 = palette.border
	watermarkFrame.BorderMode = Enum.BorderMode.Inset
	watermarkFrame.ZIndex = 1000
	watermarkFrame.Parent = screenGui
	
	local watermarkCorner = Instance.new("UICorner")
	watermarkCorner.CornerRadius = UDim.new(0, 5 * scale)
	watermarkCorner.Parent = watermarkFrame
	
	local watermarkImage = Instance.new("ImageLabel")
	watermarkImage.Size = UDim2.new(0.8, 0, 0.8, 0)
	watermarkImage.Position = UDim2.new(0.5, 0, 0.5, 0)
	watermarkImage.AnchorPoint = Vector2.new(0.5, 0.5)
	watermarkImage.BackgroundTransparency = 1
	watermarkImage.Image = WATERMARK_IMAGE_ID
	watermarkImage.ImageTransparency = 0
	watermarkImage.ScaleType = Enum.ScaleType.Fit
	watermarkImage.Parent = watermarkFrame
	
	local watermarkText = Instance.new("TextLabel")
	watermarkText.Size = UDim2.new(1, 0, 1, 0)
	watermarkText.BackgroundTransparency = 1
	watermarkText.Text = WATERMARK_FALLBACK_TEXT
	watermarkText.TextColor3 = palette.text
	watermarkText.Font = Enum.Font.GothamBold
	watermarkText.TextSize = 36 * scale
	watermarkText.TextTransparency = 1
	watermarkText.Parent = watermarkFrame
	
	local imageLoaded = false
	watermarkImage.Loaded:Connect(function()
		imageLoaded = true
		watermarkText.TextTransparency = 1
	end)
	
	watermarkImage.Failed:Connect(function()
		watermarkText.TextTransparency = 0
		watermarkImage.ImageTransparency = 1
	end)
	...
	task.delay(1.25, function()
		...
		local imageTween = tweenService:Create(watermarkImage, tweenInfo, {ImageTransparency = 1})
		local textTween = tweenService:Create(watermarkText, tweenInfo, {TextTransparency = 1})
		local frameTween = tweenService:Create(watermarkFrame, tweenInfo, {BackgroundTransparency = 1})
		...
		task.delay(fadeTime, function()
			...
			gui.dashboard.createActualDashboard()
		end)
	end)
end

function gui.dashboard.createActualDashboard()
	local scale = getScale()
	local screenSize = workspace.CurrentCamera.ViewportSize
	...
	containerFrame.Name = "CommandDashboard"
	containerFrame.Size = UDim2.new(0, dashboardWidth, 0, titleBarHeight)
	containerFrame.Position = UDim2.new(1, -dashboardWidth - (10 * scale), 1, -(titleBarHeight + 10 * scale))
	containerFrame.BackgroundColor3 = palette.windowBg
	containerFrame.BackgroundTransparency = 0.05
	containerFrame.BorderSizePixel = 1
	containerFrame.BorderColor3 = palette.border
	containerFrame.BorderMode = Enum.BorderMode.Inset
	containerFrame.ZIndex = 1000
	containerFrame.ClipsDescendants = true
	containerFrame.Parent = screenGui
	
	local containerCorner = Instance.new("UICorner")
	containerCorner.CornerRadius = UDim.new(0, 5 * scale)
	containerCorner.Parent = containerFrame
	
	dashboardState.containerFrame = containerFrame
	...
	game:GetService("UserInputService").InputChanged:Connect(function(input)
		if dashboardState.isDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			local delta = input.Position - dragStart
			
			if math.abs(delta.Y) > 10 then
				if delta.Y < -50 and not dashboardState.isExpanded then
					gui.dashboard.expand()
					dashboardState.isDragging = false
				elseif delta.Y > 50 and dashboardState.isExpanded then
					gui.dashboard.collapse()
					dashboardState.isDragging = false
				end
			end
	...
	searchContainer.Position = UDim2.new(0, 4 * scale, 0, titleBarHeight + 4 * scale)
	searchContainer.BackgroundTransparency = 1
	searchContainer.Parent = containerFrame
	...
	searchBar.Focused:Connect(function()
		dashboardState.isTyping = true
		updateInteraction()
	end)
	
	searchBar.FocusLost:Connect(function(enterPressed)
		dashboardState.isTyping = false
	end)
	
	local commandScrollFrame = Instance.new("ScrollingFrame")
	commandScrollFrame.Name = "CommandList"
	commandScrollFrame.Size = UDim2.new(1, -8 * scale, 1, -(titleBarHeight + 40 * scale))
	commandScrollFrame.Position = UDim2.new(0, 4 * scale, 0, titleBarHeight + 36 * scale)
	...
	commandScrollFrame.Parent = containerFrame
	
	commandScrollFrame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			updateInteraction()
		end
	end)
	...
	task.spawn(function()
		while dashboardState.containerFrame do
			task.wait(0.5)
			if dashboardState.isExpanded and not dashboardState.isTyping and not dashboardState.isDragging then
				if tick() - dashboardState.lastInteractionTime > 5 then
					gui.dashboard.collapse()
				end
			end
		end
