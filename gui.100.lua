...
local WATERMARK_IMAGE = "rbxassetid://0"
...
local function animateOpen()
	if not dashboardState.window or dashboardState.watermarkActive then return end
	...
	local targetPos = UDim2.new(1, -(dashboardWidth + 10 * scale), 1, -(dashboardHeight + 10 * scale))
	
	local tweenService = game:GetService("TweenService")
	local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	
	dashboardState.window.Visible = true
	local tween = tweenService:Create(dashboardState.window, tweenInfo, {Position = targetPos, Size = UDim2.new(0, dashboardWidth, 0, dashboardHeight)})
	tween:Play()
...
local function animateClose()
	...
	local targetPos = UDim2.new(1, -(60 * scale), 1, -(60 * scale))
	
	local tweenService = game:GetService("TweenService")
	local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
	
	local tween = tweenService:Create(dashboardState.window, tweenInfo, {Position = targetPos, Size = UDim2.new(0, 50 * scale, 0, 50 * scale)})
	tween:Play()
	
	tween.Completed:Connect(function()
		dashboardState.window.Visible = false
		if dashboardState.minimizedButton then
			dashboardState.minimizedButton.Visible = true
		end
	end)
...
function gui.dashboard.toggle()
	if dashboardState.watermarkActive then return end
	
	if dashboardState.isMinimized then
		gui.dashboard.open()
	else
		gui.dashboard.minimize()
	end
end

function gui.dashboard.open()
	if not dashboardState.isMinimized or dashboardState.watermarkActive then return end
	
	if dashboardState.minimizedButton then
		dashboardState.minimizedButton.Visible = false
	end
...
function gui.dashboard.minimize()
	if dashboardState.isMinimized or dashboardState.watermarkActive then return end
	
	animateClose()
end

function gui.dashboard.isOpen()
	return not dashboardState.isMinimized
end
...
function gui.dashboard.destroy()
	...
	if dashboardState.minimizedButton then
		dashboardState.minimizedButton:Destroy()
	end
...
function gui.dashboard.init()
	...
	watermark.Parent = windowData.frame
	
	uiElements.watermark = watermark
	
	local searchContainer = Instance.new("Frame")
	searchContainer.Name = "SearchContainer"
	searchContainer.Size = UDim2.new(1, -8 * scale, 0, 28 * scale)
	searchContainer.Position = UDim2.new(0, 4 * scale, 0, 4 * scale)
	...
	commandScrollFrame.Size = UDim2.new(1, -8 * scale, 1, -(40 * scale))
	commandScrollFrame.Position = UDim2.new(0, 4 * scale, 0, 36 * scale)
	...
	minimizedButton.Name = "MinimizedDashboard"
	minimizedButton.Size = UDim2.new(0, 50 * scale, 0, 50 * scale)
	minimizedButton.Position = UDim2.new(1, -(60 * scale), 1, -(60 * scale))
	...
	minimizedButton.TextSize = 24 * scale
	...
	dashboardState.minimizedButton = minimizedButton
	...
	minimizedButton.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			buttonDragStart = minimizedButton.Position
		end
	end)
	...
	game:GetService("UserInputService").InputChanged:Connect(function(input)
		if isDraggingButton and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			local newPosition = UDim2.new(
				buttonDragStart.X.Scale,
				buttonDragStart.X.Offset + delta.X,
				buttonDragStart.Y.Scale,
				buttonDragStart.Y.Offset + delta.Y
			)
	...
	local tweenService = game:GetService("TweenService")
	local fadeOutTween = tweenService:Create(watermark, TweenInfo.new(2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 1})
	...
	task.spawn(function()
		...
		watermark:Destroy()
		uiElements.watermark = nil
		dashboardState.watermarkActive = false
	...
	task.spawn(function()
		while true do
			...
			if not dashboardState.isMinimized and not dashboardState.watermarkActive then
				local elapsed = tick() - dashboardState.lastInteractionTime
				
				if elapsed >= dashboardState.autoCloseTimeout then
					gui.dashboard.minimize()
				end
			end
		end
